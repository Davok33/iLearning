...
#creamos las vistas de empleados nos va a mostrar nombre, sexo, seccion y cantidad de hijos

select * from vista_empleado;

select seccion, count(*) as cantidad from vista_empleado group by seccion;
#hace una vista de los empleados que hay en cada seccion 'Administracion - 2', ...

drop view if exists vista_empleado;
#borrar vista que se ha creado
...
#crea una vista llamada vista_empleado_ingreso de los empleados que ingresaron en cada fecha y la cantidad
...
#crea una vista llamada vista_empleado_con_hijos que la cantidad de hijos sea mayor a 00
...
drop table if exists profesores;
 ...
 where nota >= 7;
 #Crea una vista 'vista_nota_alumnos_aprodabos' sentencia select, una vista de los alumnos que han aprobado con una nota de >= 7
 ...
 #add a la vista 'vista_nota_alumnos_aprodabos' un nuevo alumno con su nota
 
  insert into vista_nota_alumnos_aprodabos(documento, nombrealumno, nota, codigoprofesor)
 values('86586932', 'Alejandro Martinez', 5, 1);
 #add otro alumno a la vista pero este no se muestra porque la nota es de 5
 
 update vista_nota_alumnos_aprodabos set nota=10 where documento='30444444';
 #actualizamos los datos del documento '30444444' que es Diana y le añadimos la nota 10
